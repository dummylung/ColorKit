...
extension UIImage {
    ...
    public func dominantColors(with quality: DominantColorQuality = .fair, algorithm: DominantColorAlgorithm = .iterative, dominantColorsMaxCount: Int = 8) throws -> [UIColor] {
        switch algorithm {
        case .iterative:
            let dominantColorFrequencies = try self.dominantColorFrequencies(with: quality, dominantColorsMaxCount: dominantColorsMaxCount)
        ...
        case .kMeansClustering:
            let dominantcolors = try kMeansClustering(with: quality, clusterCount: dominantColorsMaxCount)
            return dominantcolors
        }
    }
    ...
    public func dominantColorFrequencies(with quality: DominantColorQuality = .fair, dominantColorsMaxCount: Int = 8) throws -> [ColorFrequency] {
        ...
        let componentSize = (cgImage.bitsPerPixel / cgImage.bitsPerComponent)
        
        for yCoordonate in 0 ..< cgImage.height {
            for xCoordonate in 0 ..< cgImage.width {
                
                let index: Int = (cgImage.width * yCoordonate + xCoordonate) * componentSize
    ...
    private func kMeansClustering(with quality: DominantColorQuality, clusterCount: Int = 8) throws -> [UIColor] {
